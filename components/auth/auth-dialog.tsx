...
export function AuthDialog({ open, onOpenChange, defaultTab = "signin" }: AuthDialogProps) {
  ...
  const { signIn, signUp, signInWithGoogle, resetPassword, enableGuestMode } = useAuth()
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Welcome to MarkovLearn</DialogTitle>
          <DialogDescription>
            Sign in to save your progress across devices, or continue as a guest.
          </DialogDescription>
        </DialogHeader>

        <Tabs value={activeTab} onValueChange={(v) => { setActiveTab(v as any); setError(null); setResetEmailSent(false); setName("") }}>
            ...
            <form onSubmit={handleSignUp} className="space-y-4">
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Optional - will use email username if not provided
                </p>
              </div>
              <div className="space-y-2">
