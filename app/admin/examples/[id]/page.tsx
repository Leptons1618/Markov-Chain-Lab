...
export default function EditExamplePage() {
      ...
      try {
        ...
        if (data.success) {
          ...
          setFormData({
            ...
            interactive_demo: ex.interactive_demo || false,
            status: ex.status,
            design: JSON.stringify(ex.design, null, 2),
            explanation: ex.explanation || "",
            lesson_connections: ex.lesson_connections ? JSON.stringify(ex.lesson_connections, null, 2) : "",
            mathematical_details: ex.mathematical_details ? JSON.stringify(ex.mathematical_details, null, 2) : "",
            real_world_context: ex.real_world_context || "",
            practice_questions: ex.practice_questions ? JSON.stringify(ex.practice_questions, null, 2) : "",
          })
        } else {
          setFormError(data.error || "Failed to load example")
        }
      } catch (err: any) {
        setFormError(err.message || "Failed to load example")
      } finally {
        setLoading(false)
      }
    }
  ...
  const removeApplication = (index: number) => {
    setFormData((prev: any) => ({
      ...prev,
      applications: prev.applications.filter((_: any, i: number) => i !== index),
    }))
  }
  ...
  const currentDesign = useMemo(() => {
    if (!formData) return parsedDesign
    try {
      ...
      // Invalid JSON, return parsedDesign
    }
    return parsedDesign
  }, [formData, parsedDesign])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      success("Example updated successfully!")
      router.push("/admin/examples")
    } catch (err: any) {
      console.error("Update error:", err)
      setFormError(err.message || "Failed to update example")
      error(err.message || "Failed to update example")
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12">
        ...
        <span className="text-muted-foreground">Loading example...</span>
      </div>
    )
  }
    ...
    return (
      <div className="space-y-4">
        ...
        <Card className="p-8 text-center">
          <p className="text-muted-foreground">{formError || "Example not found"}</p>
        </Card>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Update the details for this example</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input id="id" value={formData.id} disabled className="cursor-not-allowed" />
              </div>
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                onChange={(e) => setFormData((prev: any) => ({ ...prev, interactive_demo: e.target.checked }))}
